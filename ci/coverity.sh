...
# SLUG                              - the boostorg project name
...
if [[ "$1" != "--skipdownload" ]]; then
  ...
  wget https://scan.coverity.com/download/linux64 --post-data "token=$COVERITY_SCAN_TOKEN&project=boostorg/$SLUG" -O coverity_tool.tgz
...
curl --form token="$COVERITY_SCAN_TOKEN" \
     ...
     --form description="boostorg/$SLUG" \
     https://scan.coverity.com/builds?project="boostorg/$SLUG"
